...
/**
 * CDP Auth Middleware - shared authentication for all CDP routes.
 * Eliminates auth code duplication across /json/version, /json/list, /json, and /.
 */
cdp.use('*', async (c, next) => {
...
/**
 * Build CDP WebSocket URL from request context
 */
function buildCdpWsUrl(c: { req: { url: string } }): string {
  const url = new URL(c.req.url);
...
/**
 * Build CDP target list payload
 */
function buildTargetList(wsUrl: string) {
  return [
    {
      description: '',
      devtoolsFrontendUrl: '',
...
/** Supported CDP methods list (cached constant to avoid recreation) */
const CDP_SUPPORTED_METHODS = [
  'Browser.getVersion', 'Browser.close',
  'Target.createTarget', 'Target.closeTarget', 'Target.getTargets', 'Target.attachToTarget',
  'Page.navigate', 'Page.reload', 'Page.captureScreenshot', 'Page.getFrameTree',
  'Page.getLayoutMetrics', 'Page.bringToFront', 'Page.setContent', 'Page.printToPDF',
  'Page.addScriptToEvaluateOnNewDocument', 'Page.removeScriptToEvaluateOnNewDocument',
  'Page.handleJavaScriptDialog', 'Page.stopLoading', 'Page.getNavigationHistory',
  'Page.navigateToHistoryEntry', 'Page.setBypassCSP',
  'Runtime.evaluate', 'Runtime.callFunctionOn', 'Runtime.getProperties',
  'Runtime.releaseObject', 'Runtime.releaseObjectGroup',
  'DOM.getDocument', 'DOM.querySelector', 'DOM.querySelectorAll', 'DOM.getOuterHTML',
  'DOM.getAttributes', 'DOM.setAttributeValue', 'DOM.focus', 'DOM.getBoxModel',
  'DOM.scrollIntoViewIfNeeded', 'DOM.removeNode', 'DOM.setNodeValue', 'DOM.setFileInputFiles',
  'Input.dispatchMouseEvent', 'Input.dispatchKeyEvent', 'Input.insertText',
  'Network.enable', 'Network.disable', 'Network.setCacheDisabled', 'Network.setExtraHTTPHeaders',
  'Network.setCookie', 'Network.setCookies', 'Network.getCookies', 'Network.deleteCookies',
  'Network.clearBrowserCookies', 'Network.setUserAgentOverride',
  'Fetch.enable', 'Fetch.disable', 'Fetch.continueRequest', 'Fetch.fulfillRequest',
  ...
  'Emulation.clearGeolocationOverride', 'Emulation.setTimezoneOverride',
  'Emulation.setTouchEmulationEnabled', 'Emulation.setEmulatedMedia',
 ...
 * Auth handled by shared middleware above.
 */
cdp.get('/', async (c) => {
  ...
  if (upgradeHeader?.toLowerCase() !== 'websocket') {
    return c.json({
      ...
      supported_methods: CDP_SUPPORTED_METHODS,
    });
  }
 ...
 * Auth handled by shared middleware.
 */
cdp.get('/json/version', async (c) => {
  return c.json({
    ...
    webSocketDebuggerUrl: buildCdpWsUrl(c),
  });
});
...
cdp.get('/json/list', async (c) => {
  return c.json(buildTargetList(buildCdpWsUrl(c)));
});
