...
// Middleware: Log every request (conditional verbosity for performance)
app.use('*', async (c, next) => {
  const isDebug = c.env.DEBUG_ROUTES === 'true';
  if (isDebug) {
...
// These are exempted from Access protection (health checks, static assets)
// =============================================================================
...
// Access is enabled for the ENTIRE domain. All routes below require a valid
// CF Access JWT. The Worker validates the JWT and auto-injects the gateway
// token so the user never needs ?token= in the URL.
// =============================================================================
...
// Middleware: Cloudflare Access authentication for ALL protected routes
// Access is enabled for the entire domain, so every request should have a JWT.
// We validate it to confirm the user is authorized, then inject the gateway token.
...
// Apply Access auth to ALL remaining routes (entire domain is Access-protected)
app.use('*', accessMiddleware);

// Mount API routes
app.route('/api', api);

// Mount Admin UI routes (protected by Cloudflare Access middleware above)
app.route('/_admin', adminUi);

// Mount debug routes (protected by Cloudflare Access + DEBUG_ROUTES flag)
...
app.all('*', async (c) => {
  ...
  if (debugLogs) console.log('[PROXY] Handling request:', url.pathname);
  ...
  if (!isGatewayReady && !isWebSocketRequest && acceptsHtml) {
    if (debugLogs) console.log('[PROXY] Gateway not ready, serving loading page');
  ...
  if (isWebSocketRequest) {
    ...
    // ALWAYS inject the gateway token for WebSocket connections.
    // Since Cloudflare Access protects the entire domain, authenticated users
    // never need to pass ?token= manually. The Worker handles it server-side.
  ...
  // Inject gateway token for HTTP requests too (Access-authenticated users don't have ?token=)
  ...
  if (debugLogs) console.log('[HTTP] Proxying:', url.pathname + url.search);
  const httpResponse = await sandbox.containerFetch(httpRequest, MOLTBOT_PORT);
  if (debugLogs) console.log('[HTTP] Response status:', httpResponse.status);
