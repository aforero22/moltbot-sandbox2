...
RCLONE_FLAGS="--transfers=16 --fast-list --s3-no-check-bucket --checkers=8"
...
# ============================================================
# BACKGROUND SYNC LOOP (optimized: inotifywait-based or longer interval)
# ============================================================
if r2_configured; then
    echo "Starting background R2 sync loop (optimized)..."
    (
        MARKER=/tmp/.last-sync-marker
        LOGFILE=/tmp/r2-sync.log
        touch "$MARKER"

        # Initial sync interval: 60s (up from 30s) to reduce CPU/API calls
        # Only sync when files actually changed
    ...
    echo "Background sync loop started (PID: $!, interval: ${SYNC_INTERVAL:-60}s)"
fi
